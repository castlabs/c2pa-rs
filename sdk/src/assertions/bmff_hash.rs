...
/// Merkle UUID proof boxes that [`BmffHash::add_merkle_map_for_mdats`] says
/// need to be spliced into the asset, and where. See
/// [`BmffHash::merkle_uuid_boxes`].
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct MerkleUuidBoxes<'a> {
    /// The flattened UUID box bytes to insert.
    pub bytes: &'a [u8],
    /// Byte offset in the asset to insert `bytes` at — immediately after the
    /// last mdat box.
    pub insertion_point: u64,
    /// Length, in bytes, of a pre-existing set of Merkle UUID boxes at
    /// `insertion_point` that should be overwritten rather than inserted
    /// after — nonzero only when resigning an asset that already has them.
    pub replacement_range: u64,
}
...
impl BmffHash {
    ...
    /// Fills in real Merkle maps from an accumulator that collected leaf
    /// hashes
    ///
    /// Any pending fixed-size remainder (a partially filled last leaf) is
    /// folded in first. Unlike [`add_mdat_leaf_hashes`](Self::add_mdat_leaf_hashes),
    /// this supports proof rows (not just the Merkle root), since the
    /// accumulator's leaves retain their per-leaf lengths needed to rebuild
    /// the tree shape.
    ///
    /// The `/mdat` exclusion is added so mdat content is excluded from the
    /// flat hash (it is covered by the Merkle trees instead). No-op if the
    /// accumulator has no leaves (i.e. `add_merkle_leaf` was never called).
    pub fn add_merkle_maps_from_accumulator(
        &mut self,
        accumulator: &mut MerkleAccumulator,
    ) -> crate::Result<()> {
        for (mdat_id, remainder) in &accumulator.fixed_size_remainder {
            let fragment_hash = hash_by_alg(accumulator.alg.as_str(), remainder, None);
    ...
    /// Merkle UUID proof boxes that need to be spliced into the asset,
    /// computed by [`add_merkle_map_for_mdats`](Self::add_merkle_map_for_mdats)
    /// when `max_proofs` calls for storing a proof row rather than just the
    /// Merkle root. `None` if no boxes are needed (e.g. a single-leaf tree,
    /// or `max_proofs == 0`) — nothing to insert.
    pub fn merkle_uuid_boxes(&self) -> Option<MerkleUuidBoxes<'_>> {
        self.merkle_uuid_boxes
            .as_deref()
            .map(|bytes| MerkleUuidBoxes {
                bytes,
                insertion_point: self.merkle_uuid_boxes_insertion_point,
                replacement_range: self.merkle_replacement_range,
            })
    }
...
mod bmff_hash_tests {
    ...
    /// Merkle tree directly from a stream, splice the resulting Merkle UUID
    /// proof boxes into the asset using the public
    /// [`BmffHash::merkle_uuid_boxes`] accessor (mirroring exactly what
    /// `Store::generate_bmff_mdat_hashes` does internally), then hash the
    /// patched asset and verify the result round-trips.
    ///
    /// This assumes the fixture's last mdat box is also the last box in the
    /// file (a common "streaming-friendly" `ftyp, moov, mdat` layout), so
    /// splicing after it doesn't shift or invalidate any later structure. A
    /// fixture with trailing boxes after the last mdat (e.g. `mfra`) would
    /// need those adjusted too — same as `Store`'s own `insert_data_at` usage.
    #[test]
    fn test_build_hard_binding_externally_with_merkle_proof_boxes() {
        use std::io::{Read, Write};

        use crate::utils::test::fixture_path;

        let path = fixture_path("video1_no_manifest.mp4");
        ...
        // Small chunk size on a real, several-hundred-KB video forces a Merkle
        // tree deep enough that, combined with the default-sized max_proofs,
        // a proof row (not just the root) must be stored as UUID boxes.
        let max_proofs = 5;
        bmff_hash
            .add_merkle_map_for_mdats(&mut source, 16, max_proofs)
            .unwrap();
        assert!(
            bmff_hash.merkle().is_some(),
            "Merkle tree should be computed"
        );

        let uuid_boxes = bmff_hash
            .merkle_uuid_boxes()
            .expect("a multi-chunk file with max_proofs > 0 should need UUID proof boxes");
        assert!(!uuid_boxes.bytes.is_empty());
        assert!(uuid_boxes.insertion_point > 0);
        // Fresh asset, no pre-existing Merkle UUID boxes to overwrite.
        assert_eq!(uuid_boxes.replacement_range, 0);

        // Verify the insertion point really is the end of the file for this
        // fixture, so splicing there doesn't shift/invalidate anything after.
        let source_len = source.metadata().unwrap().len();
        assert_eq!(
            uuid_boxes.insertion_point, source_len,
            "test fixture assumption: last mdat box must be the last box in the file"
        );

        // Splice the boxes into the asset ourselves, using only the exposed
        // getters — no crate-internal helper (e.g. `insert_data_at`) needed.
        source.rewind().unwrap();
        let mut before = vec![0u8; uuid_boxes.insertion_point as usize];
        source.read_exact(&mut before).unwrap();

        let mut patched = Cursor::new(Vec::new());
        patched.write_all(&before).unwrap();
        patched.write_all(uuid_boxes.bytes).unwrap();
        patched.rewind().unwrap();

        // Compute the flat hash over the patched asset (mdat is covered by
        // the Merkle trees, so it's excluded here via the assertion's own
        // exclusion list).
        bmff_hash.gen_hash_from_stream(&mut patched).unwrap();
        assert!(bmff_hash.hash().is_some());

        patched.rewind().unwrap();
        bmff_hash
            .verify_stream_hash(&mut patched, Some("sha256"))
            .expect("hard binding constructed via the public API should verify");
    }
}
