...
impl Store {
    ...
    fn write_dynamic_assertions(
            ...
            .zip(resolved_labels.iter())
        {
            ...
            let pc = self.provenance_claim_mut().ok_or(Error::ClaimEncoding)?;
            pc.replace_assertion_instance(assertion, target_instance)?;
            modified = true;
    ...
    fn requires_embedded_manifest_layout_match(is_bmff: bool, claim: &Claim) -> bool {
    ...
    pub(crate) fn save_to_stream(
        ...
        let enforce_embedded_manifest_layout = {
            let pc = self.provenance_claim().ok_or(Error::ClaimEncoding)?;
            Self::requires_embedded_manifest_layout_match(context.io().is_bmff_format(format), pc)
        };
...
pub mod tests {
    ...
    struct ReservedDynamicSigner {
        signer: crate::signer::BoxedSigner,
        output: ReservedDynamicOutput,
    }
    ...
    */
    ...
    fn test_embedded_manifest_layout_match_classification() {
        ...
        assert!(Store::requires_embedded_manifest_layout_match(
            Context::new().io().is_bmff_format("image/jpeg"),
            &data_hash
        ));
        ...
        assert!(Store::requires_embedded_manifest_layout_match(
            Context::new().io().is_bmff_format("video/mp4"),
            &data_and_box_hash
        ));
    ...
    fn test_embedded_data_hash_exact_dynamic_assertion_preserves_layout() {
        ...
        assert!(Store::requires_embedded_manifest_layout_match(
            Context::new().io().is_bmff_format(format),
            claim
        ));
    ...
    fn test_sign_manifest_enforces_dynamic_assertion_reservation_for_box_hash() {
        ...
        let mut short_context = Context::new().with_signer(short_signer);
        short_context.settings_mut().verify.verify_after_sign = false;
        let mut short_store = box_hash_store(&context);
        let error = short_store
            .sign_manifest(&short_context, None)
        ...
        let mut exact_context = Context::new().with_signer(exact_signer);
        exact_context.settings_mut().verify.verify_after_sign = false;
        let mut exact_store = box_hash_store(&context);
        let jumbf = exact_store
            .sign_manifest(&exact_context, None)
